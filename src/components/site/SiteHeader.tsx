...
import { usePhone, useSettings } from "@/components/site/SettingsProvider";
...
const NAV: NavItem[] = [
  {
    ...
    children: [
      {
        label: "Hearing Evaluation (Adult & Pediatric)",
        to: "/services/hearing-evaluation",
      },
      {
        label: "Hearing Aid Counseling & Fitting",
        to: "/services/hearing-aid-fittings",
      },
      {
        label: "Hearing Aid Repair & Maintenance",
        to: "/services/hearing-aid-repairs",
      },
      { label: "Follow-up Care & Counseling", to: "/services/follow-up-care" },
      {
        label: "Assistive Listening Devices & Accessories",
        to: "/services/assistive-listening-devices",
      },
    ],
  },
  {
  ...
  if (!item.children) {
    return (
      <Link
        href={item.to!}
        className={
          "rounded-full px-3 py-2 text-sm font-semibold text-foreground hover:bg-muted" +
          activeCls
        }
      >
        {item.label}
  ...
  return (
    <div className="group relative">
      <Link
        href={item.to!}
        className={
          "inline-flex items-center gap-1 rounded-full px-3 py-2 text-sm font-semibold text-foreground hover:bg-muted" +
          activeCls
        }
      >
        {item.label}
